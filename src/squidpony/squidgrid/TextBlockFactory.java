...
/**
 * Singleton class for creating text blocks. Stores images in a TreeMap to prevent
 * duplicate object creation where not needed. For applications with many
 * repeated text blocks, this is an ideal way to hold and create them.
 *
 * @author SquidPony
 */
public class TextBlockFactory {
     ...
     * passed in. The font size in the font will be ignored.
     * 
     * @param width
     * @param height
     * @param font 
    ...
    /**
     * Makes the cell as small as possible with the basic plane printable ASCII characters
     * still fitting inside.
     * 
    ...
    private void sizeCellByDimension(boolean whiteSpace) {
        int fontSize = width * height;//start guaranteed to be too large for the cell TODO -- find a better way to minimize the starting point
    ...
    /**
     * Returns true if the given character will fit inside the current cell dimensions
     * with the current font. ISO Control characters are considered to not fit by definition.
     * 
     * @param c
     * @param whiteSpace if true then there must be room for the character c and at least one empty pixel on all sides of the character
     * @return 
     */
    public boolean willFit(char c, boolean whiteSpace) {
        ...
        g.drawString("" + c, (width + 2 + 2 * testWidth - g.getFontMetrics().charWidth(c)) / 2, g.getFontMetrics().getMaxAscent() - verticalOffset + 1 + testWidth);

        //check for font drawn at the edge
        leftCheckLoop:
        for (int x = 0; x <= testWidth; x++) {//check for left side
        ...
        rightCheckLoop:
        for (int x = width - 1; x >= width - testWidth - 1; x--) {//check for right side
            for (int y = 0; y < height;) {
                if (testImage.getRGB(x, y) != Color.WHITE.getRGB()) {
                    return false;
                }
        ...
        topCheckLoop:
        for (int y = 0; y < testHeight;) {//check for top
            for (int x = 0; x < width; x++) {
                if (testImage.getRGB(x, y) != Color.WHITE.getRGB()) {
                    return false;
                }
        ...
        bottomCheckLoop:
        for (int y = testHeight - 1; y >= 0; y--) {//check for bottom
            for (int x = 0; x < width; x++) {
                if (testImage.getRGB(x, y) != Color.WHITE.getRGB()) {
                    return false;
                }
